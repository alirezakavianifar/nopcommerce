...
(async () => {
    ...
    console.log('3. Navigating to seller dashboard...');
    const response = await page.goto('http://localhost:59580/seller/dashboard', { waitUntil: 'domcontentloaded' });
    console.log('Response status:', response.status());

    await page.evaluate(() => new Promise(r => setTimeout(r, 2000)));

    console.log('4. Saving clean Persian seller dashboard screenshot...');
    await page.screenshot({ path: path.join(outputDir, 'step3_seller_dashboard.png'), fullPage: false });
    console.log('Saved 100% Persian step3_seller_dashboard.png!');
